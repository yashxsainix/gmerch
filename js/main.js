...
function initShopPage() {
  ...
  document.querySelectorAll('[data-size-filter]').forEach((button) => {
    button.addEventListener('click', () => {
      appState.filters.size = button.dataset.sizeFilter === 'All' ? 'M' : button.dataset.sizeFilter;
  ...
  document.querySelectorAll('[data-color-filter]').forEach((button) => {
    button.addEventListener('click', () => {
      appState.filters.color = button.dataset.colorFilter === 'All' ? 'Pink' : button.dataset.colorFilter;
  ...
  document.getElementById('reset-filters')?.addEventListener('click', () => {
    appState.filters = { size: 'M', color: 'Pink', priceMax: 35 };
    document.querySelectorAll('[data-size-filter]').forEach((pill) => pill.classList.toggle('active', pill.dataset.sizeFilter === 'M'));
    document.querySelectorAll('[data-color-filter]').forEach((pill) => pill.classList.toggle('active', pill.dataset.colorFilter === 'Pink'));
...
function getProductSelection(productId) {
  return appState.productSelections[productId] || {
    size: appState.filters.size,
    color: appState.filters.color
  };
}
...
function renderProducts() {
  ...
  const filtered = PRODUCTS.filter((product) => {
    return product.type === appState.shopType && product.price <= appState.filters.priceMax;
  });
